...
func (c *Client) CallContext(ctx context.Context, result interface{}, method string, args ...interface{}) error {
	...
	case len(resp.Result) == 0:
		return ErrNoResult
	default:
		if _, ok := result.(**types.Header); ok {
			jsonMap := make(map[string]interface{})
			if err := json.Unmarshal(resp.Result, &jsonMap); err != nil {
				return err
			}
			if logsBloom, ok := jsonMap["logsBloom"].(string); ok && logsBloom == "0x" {
				jsonMap["logsBloom"] = "0x" + strings.Repeat("0", 512)
			}
			resp.Result, _ = json.Marshal(jsonMap)
		}
